...
export default function Panier() {
  ...
  const [cart, setCart] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('kp_cart') || '[]');
      // Nettoyer les items avec prix corrompu
      const clean = saved.filter(i => Number(i.price ?? i.unit_price ?? 0) > 0);
      return clean;
    } catch { return []; }
  });
  ...
  const commander = async () => {
    ...
    try {
      ...
      unit_price: Number(String(i.price ?? i.unit_price ?? 0).replace(/\s/g, '')),
    }));
