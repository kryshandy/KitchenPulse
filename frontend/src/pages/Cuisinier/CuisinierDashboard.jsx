...
// ─── Corrections v2 ───────────────────────────────────────────
//  ✓ Plus de react-toastify → toast inline
//  ✓ Tous les endpoints corrigés (/api/dishes, /api/orders)
//  ✓ Status update via PATCH /api/orders/:id/status
//  ✓ Catégories chargées depuis GET /api/dishes/categories
...
// ─── Toast inline (remplace react-toastify) ───────────────────
function useToast() {
...
function ToastContainer({ toasts, T }) {
  const colors = {
    success: { bg: T.green,  text: "#0A0C10" },
    error:   { bg: T.red,    text: "#fff"    },
    warn:    { bg: T.accent, text: "#0A0C10" },
  };
  return (
    <div style={{ position: "fixed", top: 16, right: 16, zIndex: 9999, display: "flex", flexDirection: "column", gap: 8 }}>
      {toasts.map(t => {
        ...
        return (
          <div key={t.id} style={{
            background: c.bg, color: c.text, padding: "10px 16px", borderRadius: 10,
...
const Btn = ({ children, onClick, variant = "accent", size = "md", disabled, full, style: sx = {}, T }) => {
  const variants = {
    ...
    amber:   { background: T.accent,  color: "#0A0C10", border: "none" },
  ...
  return (
    <button onClick={disabled ? undefined : onClick} style={{
      ...v, padding: sizes[size], borderRadius: 10, fontWeight: 600, cursor: disabled ? "not-allowed" : "pointer",
...
const Field = ({ label, required, T, textarea, ...props }) => {
  ...
  const base = {
    ...
    background: T.bg, border: `1.5px solid ${focused ? T.accent : T.border}`,
    borderRadius: 9, color: T.text, fontSize: 13, transition: "border-color .2s",
    boxSizing: "border-box", resize: "none",
  };
  return (
    <div style={{ marginBottom: 12 }}>
      {label && (
        <label style={{ display: "block", fontSize: 10, color: T.muted, fontWeight: 700,
...
// ─── Timer (commandes EN_PREPARATION) ─────────────────────────
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const enriched = data.map(o => ({
        ...o,
        _startTs: o.status === "EN_PREPARATION" && o.updated_at
          ? new Date(o.updated_at).getTime() : null,
      }));
      setOrders(enriched);
    } catch {
    ...
    <div style={{ padding: "16px 16px 90px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <button onClick={() => load(true)} style={{
          background: T.surface, border: `1px solid ${T.border}`, color: T.sub,
          borderRadius: 9, padding: "8px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontSize: 12,
        }}>
      ...
      {displayed.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 0", color: T.muted }}>
          <div style={{ marginBottom: 12, color: T.border }}>
...
const GestionMenu = ({ toast, T }) => {
  ...
  // ✅ Envoi multipart/form-data vers /api/dishes
  const save = async () => {
  ...
  const toggleActive = async (dish) => {
    try {
      await api.patch(`/dishes/${dish.id}`, { is_active: !dish.is_active });
      toast.success(dish.is_active ? "Plat masqué du menu" : "Plat activé");
      loadAll();
  ...
  return (
    <div style={{ padding: "16px 16px 90px" }}>
      ...
      {/* Filtres catégories ← chargés depuis la BD */}
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: 40, color: T.muted, fontSize: 14 }}>
          {search ? `Aucun résultat pour « ${search} »` : "Aucun plat dans cette catégorie"}
        </div>
      ) : (
        filtered.map(dish => (
          <div key={dish.id} style={{
            background: T.card, border: `1px solid ${T.border}`, borderRadius: 14,
            padding: 14, marginBottom: 10, opacity: dish.is_active ? 1 : 0.5,
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div style={{ width: 56, height: 56, borderRadius: 12, overflow: "hidden", flexShrink: 0, background: T.surface, border: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", color: T.muted }}>
                {dish.image_url
                  ? <img src={`http://localhost:3001${dish.image_url}`} alt={dish.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { e.target.style.display = "none"; e.target.parentNode.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>'; }} />
                  : Ic.img
                }
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: T.text, marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {dish.name}
                  {dish.is_featured ? <Tag color={T.accent} bg={`${T.accent}20`} style={{ marginLeft: 6 }}>Vedette</Tag> : null}
                </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", zIndex: 200, display: "flex", alignItems: "flex-end", backdropFilter: "blur(4px)" }}
          ...
          <div style={{ background: T.card, borderRadius: "20px 20px 0 0", width: "100%", maxHeight: "92vh", overflowY: "auto", padding: "20px 20px 40px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <button onClick={() => setShowModal(false)} style={{ background: T.surface, border: "none", color: T.muted, width: 32, height: 32, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {Ic.close}
              </button>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              <label style={{ display: "block", fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: .6, textTransform: "uppercase", marginBottom: 8 }}>
                Photo du plat
              </label>
              <div onClick={() => fileRef.current?.click()} style={{
                border: `2px dashed ${imgPrev ? T.accent : T.border}`, borderRadius: 12,
                minHeight: 90, display: "flex", flexDirection: "column", alignItems: "center",
                justifyContent: "center", cursor: "pointer", overflow: "hidden",
              }}>
                {imgPrev
                  ? <img src={imgPrev} alt="" style={{ width: "100%", maxHeight: 180, objectFit: "cover" }} />
                  : <>
                      <span style={{ color: T.muted, marginBottom: 6 }}>{Ic.upload}</span>
                      <span style={{ fontSize: 12, color: T.muted }}>Cliquer pour choisir une image</span>
                      <span style={{ fontSize: 11, color: T.border, marginTop: 2 }}>JPG, PNG, WEBP — max 5 Mo</span>
                    </>
                }
              </div>
              ...
              {imgPrev && (
                <button onClick={() => { setImgFile(null); setImgPrev(null); }}
                  style={{ marginTop: 6, fontSize: 11, color: T.muted, background: "none", border: "none", cursor: "pointer" }}>
                  ✕ Supprimer l'image
                </button>
              )}
            </div>
            ...
            {/* Catégorie depuis la BD */}
            <div style={{ marginBottom: 12 }}>
              <label style={{ display: "block", fontSize: 10, color: T.muted, fontWeight: 700, letterSpacing: .6, textTransform: "uppercase", marginBottom: 6 }}>
                Catégorie<span style={{ color: T.accent, marginLeft: 2 }}>*</span>
              </label>
              <select value={form.category_id} onChange={f("category_id")} style={{
                width: "100%", padding: "11px 12px", background: T.bg,
                border: `1.5px solid ${T.border}`, borderRadius: 9,
                color: form.category_id ? T.text : T.muted, fontSize: 13,
              }}>
              ...
              {categories.length === 0 && (
                <p style={{ fontSize: 11, color: T.red, marginTop: 4 }}>
                  Aucune catégorie trouvée — vérifiez que la table `categories` est peuplée dans kitchenpulse_db
                </p>
              )}
            </div>
            ...
            <Btn T={T} onClick={save} disabled={saving} variant="accent" full size="lg">
              {saving ? "Enregistrement…" : editDish ? `${Ic.check} Enregistrer les modifications` : `${Ic.plus} Ajouter au menu`}
            </Btn>
          </div>
        </div>
      )}
  ...
  return (
    <div style={{ position: "sticky", top: 0, zIndex: 50, background: T.surface, borderBottom: `1px solid ${T.border}`, padding: "13px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <button onClick={toggle} title="Changer le thème" style={{ background: T.surface, border: `1px solid ${T.border}`, color: T.sub, width: 32, height: 32, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {isDark ? Ic.sun : Ic.moon}
        </button>
        <button onClick={() => { logout(); navigate("/login"); }} title="Se déconnecter" style={{ background: `${T.red}20`, border: `1px solid ${T.red}30`, color: T.red, width: 32, height: 32, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {Ic.logout}
        </button>
      </div>
    </div>
